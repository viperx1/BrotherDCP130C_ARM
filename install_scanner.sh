...
display_info() {
    ...
    echo
    log_info "Data compression:"
    log_info "  The scanner DOES compress data before sending — it uses PackBits"
    log_info "  (run-length encoding). White lines are sent as 1-byte markers."
    log_info "  However, photo/color content compresses poorly with PackBits."
    log_info "  Typical compression: 2-4x for text/line art, ~1x for photos."
    log_info "  The original Windows driver used the same compression — this is"
    log_info "  handled by the scanner hardware, not the driver software."
    echo
